...
function env(extra: Record<string, string> = {}): NodeJS.ProcessEnv {
  return {
    CITARE_SERVICE_AUTH: "0123456789abcdef0123",
    ...
    AWS_SHARED_CREDENTIALS_FILE: "/nonexistent-citare-service-test-path",
    ...extra,
...
function writeCredsFile(contents: string): string {
  const dir = mkdtempSync(join(tmpdir(), "citare-service-creds-"));
  const path = join(dir, "credentials");
  writeFileSync(path, contents, "utf8");
  return path;
}

describe("loadConfig — CITARE_CACHE_ENABLED", () => {
  it("defaults to true when unset", () => {
    const cfg = loadConfig(env());
    expect(cfg.cache.enabled).toBe(true);
  });
  it("false when CITARE_CACHE_ENABLED=false", () => {
    const cfg = loadConfig(env({ CITARE_CACHE_ENABLED: "false" }));
    expect(cfg.cache.enabled).toBe(false);
  });
  it("true when CITARE_CACHE_ENABLED=true", () => {
    const cfg = loadConfig(env({ CITARE_CACHE_ENABLED: "true" }));
    expect(cfg.cache.enabled).toBe(true);
  });
});
...
describe("loadConfig — LLM provider selection", () => {
  ...
  it("picks Anthropic when only CITARE_LLM_ANTHROPIC_API_KEY set", () => {
    const cfg = loadConfig(env({ CITARE_LLM_ANTHROPIC_API_KEY: "sk-ant-test" }));
  ...
  it("honors CITARE_LLM_BEDROCK_REGION over AWS_REGION", () => {
    const cfg = loadConfig(env({
      AWS_ACCESS_KEY_ID: "AKIA-test",
      AWS_SECRET_ACCESS_KEY: "secret-test",
      AWS_REGION: "us-east-1",
      CITARE_LLM_BEDROCK_REGION: "us-west-2",
    }));
    if (cfg.llm.provider !== "bedrock") throw new Error("wrong provider");
    expect(cfg.llm.region).toBe("us-west-2");
  });

  it("honors CITARE_LLM_BEDROCK_MODEL override (e.g. Nova Lite)", () => {
    const cfg = loadConfig(env({
      AWS_ACCESS_KEY_ID: "AKIA-test",
      AWS_SECRET_ACCESS_KEY: "secret-test",
      CITARE_LLM_BEDROCK_MODEL: "us.amazon.nova-lite-v1:0",
    }));
    if (cfg.llm.provider !== "bedrock") throw new Error("wrong provider");
  ...
  it("Bedrock wins when both Bedrock and Anthropic configured", () => {
    const cfg = loadConfig(env({
      AWS_ACCESS_KEY_ID: "AKIA-test",
      AWS_SECRET_ACCESS_KEY: "secret-test",
      CITARE_LLM_ANTHROPIC_API_KEY: "sk-ant-also-set",
    }));
...
`);
  ...
  it("freetextRescue tuning env vars flow through to the picked provider", () => {
    const cfg = loadConfig(env({
      AWS_ACCESS_KEY_ID: "AKIA-test",
      AWS_SECRET_ACCESS_KEY: "secret-test",
      CITARE_LLM_FREETEXT_MIN_SCORE: "42",
      CITARE_LLM_FREETEXT_MAX_TOKENS: "256",
    }));
    if (cfg.llm.provider !== "bedrock") throw new Error("wrong provider");
...
describe("loadConfig — strategyOverrides", () => {
  it("is empty by default", () => {
    const cfg = loadConfig(env());
    expect(cfg.citare.strategyOverrides).toEqual({});
  });

  it("parses CITARE_STRATEGY_CITOID_URL_FALLBACK=true", () => {
    const cfg = loadConfig(env({ CITARE_STRATEGY_CITOID_URL_FALLBACK: "true" }));
    expect(cfg.citare.strategyOverrides).toEqual({
      CitoidUrl: { fallback: true },
    });
  });

  it("parses CITARE_STRATEGY_LLM_ENABLED=false", () => {
    const cfg = loadConfig(env({ CITARE_STRATEGY_LLM_ENABLED: "false" }));
    expect(cfg.citare.strategyOverrides).toEqual({ Llm: { enabled: false } });
  });

  it("combines multiple fields on one strategy", () => {
    const cfg = loadConfig(
      env({
        CITARE_STRATEGY_CITOID_URL_FALLBACK: "true",
        CITARE_STRATEGY_CITOID_URL_MIN_CURRENT_BEST_CONFIDENCE: "40",
      }),
    );
    expect(cfg.citare.strategyOverrides).toEqual({
  ...
  it("ignores unrecognized strategy names silently at env layer", () => {
    ...
    const cfg = loadConfig(env({ CITARE_STRATEGY_NOT_A_REAL_ENABLED: "false" }));
    expect(cfg.citare.strategyOverrides).toEqual({});
  });
  ...
  it("tokenizes camelCase → SNAKE_CASE correctly", () => {
    const cfg = loadConfig(
      env({
        CITARE_STRATEGY_CROSS_REF_DOI_ENABLED: "false",
        CITARE_STRATEGY_SCHEMA_ORG_LD_JSON_FALLBACK: "true",
      }),
    );
    expect(cfg.citare.strategyOverrides).toEqual({
